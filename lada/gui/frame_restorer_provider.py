...
class FrameRestorerOptionsBuilder:
    def __init__(self, initial: FrameRestorerOptions | None = None):
        ...
        if initial:
            self._properties = {
                ...
                "nvidia_decode": initial.nvidia_decode,
            }
    ...
    def nvidia_decode(self, value: bool) -> 'FrameRestorerOptionsBuilder':
        self._properties["nvidia_decode"] = value
        return self
...
class FrameRestorerProvider:
    ...
    def get(self):
        ...
        if self.options.passthrough:
            return PassthroughFrameRestorer(self.options.video_metadata.video_file, nvidia_decode=self.options.nvidia_decode)
        ...
        return FrameRestorer(self.options.device,
                             ...
                             nvidia_decode=self.options.nvidia_decode)
...
class PassthroughFrameRestorer:
    def __init__(self, video_file, nvidia_decode: bool = False):
    ...
    def start(self, start_ns=0):
        self.video_reader = video_utils.VideoReader(self.video_file, nvidia_decode=self.nvidia_decode)
